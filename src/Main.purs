  ...
  fixed
    [ D.div
        ...
        , D.div (klass_ $ containerKlass <> " flex-1 relative")
            $ segments <#> ropeSegment "border-red-400 bg-red-500/40"
        ]
    ]
  where
...
makeRope n p f = unfold 1 [] $ (Just <$> p)
  where
    unfold :: Int -> Array (Event (Maybe Segment)) -> Event (Maybe Point) -> Domable lock payload
    unfold i rope head
      | i > maxLength = f rope
      | otherwise =
        let
        ...
        in
        Deku.do
          segment' <- useMemoized segment
          tail' <- useMemoized tail
